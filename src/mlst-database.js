...
class Metadata {
  ...
  writeMetadata(outPath, ...options) {
    return this.buildMetadata(...options).then(data => {
      ...
      fs.writeFile(outPath, jsonData, err => {
...
class PubMlst extends Metadata {
  ...
  _downloadFile(url, downloadPath) {
    ...
    return this.downloadTokens
      ...
      .then(token => {
        logger("trace:pubmlst:download")(
          `Downloading ${url} to ${downloadPath} (${token})`
        );
        return axios
      ...
      .then(({ token, response }) => {
        ...
        return new Promise(resolve => {
          outstream.on("close", () => {
            logger("trace:pubmlst:download")(
              `Downloaded ${url} to ${downloadPath} (${token})`
            );
  ...
  _sortFastaBySequenceLength(fastaPath) {
    ...
    seqStream.on("end", () => {
      tmp.file((err, tempFastaPath) => {
        ...
        const tempOutputfile = fs.createWriteStream(tempFastaPath);
